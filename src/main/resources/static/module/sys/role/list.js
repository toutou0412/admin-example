...
listTemplateLayui.use(['index', 'table'], function () {
    ...
    //监听搜索
    form.on('submit(LAY-sys-role-search)', function (data) {
    ...
    var active = {
        // 删除
        batchdel: function () {
            var checkStatus = table.checkStatus('LAY-sys-role-manage')
...
listTemplateLayui.define(["table", "form"], function (e) {
    ...
    data_table.render({
        ...
        // height: "full-220",
        text: "对不起，加载出现异常！"
    }), data_table.on("tool(LAY-sys-role-manage)", function (e) {
        // 操作栏各按钮方法
        var rowData = e.data;
        // 删除操作
        if ("del" === e.event) {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            ...
                            if (!result) {
                                ...
                                e.del();
                                layer.close(t);
                                data_table.reload('LAY-sys-role-manage');
                                layer.msg('已删除');
                            } else {
                                layer.msg(result.msg);
                            }
            ...
            layer.open({
                type: 2,
                title: "查看",
                content: "/sys/role/dataView?action=1&id=" + e.data.id,
                maxmin: true,
                area: ["500px", "450px"],
                btn: ["确定"],
                yes: function (e, t) {
            ...
            layer.open({
                type: 2,
                title: "编辑",
                content: "/sys/role/dataView?action=2&id=" + e.data.id,
                maxmin: true,
                area: ["500px", "450px"],
                btn: ["确定", "取消"],
