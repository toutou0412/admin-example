    ...
    var active = {
        ...
        , add: function () {
            layer.open({
                type: 2
                , title: '添加用户'
                , content: '/user/dataView?action=0'
                , maxmin: true
                , area: ["500px", "450px"]
                , btn: ['确定', '取消']
...
listTemplateLayui.define(["table", "form"], function (e) {
    ...
    data_table.render({
        ...
        // height: "full-220",
        text: "对不起，加载出现异常！"
    }), data_table.on("tool(LAY-user-back-manage)", function (e) {
        // 操作栏各按钮方法
        var rowData = e.data;
        // 删除操作
        if ("del" === e.event) {
            layer.prompt({formType: 0, title: "敏感操作，请验证口令"}, function (t, i) {
                layer.close(i), layer.confirm("确定删除用户？", function (t) {
                    //执行 Ajax 后重载
                    $.ajax({
                        url: '/user/delete',
                        data: 'ids=' + rowData.id,
                        ...
                        success: function (result) {
                            ...
                            if (!result) {
                                ...
                                e.del();
                                layer.close(t);
                                data_table.reload('LAY-user-back-manage');
                                layer.msg('已删除');
                            } else {
                                layer.msg(result.msg);
                            }
            ...
            layer.open({
                type: 2,
                title: "查看用户",
                content: "/user/dataView?action=1&id=" + e.data.id,
                maxmin: true,
                area: ["500px", "450px"],
                btn: ["确定"],
                yes: function (e, t) {
            ...
            layer.open({
                type: 2,
                title: "编辑用户",
                content: "/user/dataView?action=2&id=" + e.data.id,
                maxmin: true,
                area: ["500px", "450px"],
                btn: ["确定", "取消"],
